...
fn routing_and_data_survive_restart() {
...

    {
        ...
        assert!(!snap_entries.is_empty(), "router_snapshot has size but no entries");
        assert!(!upd_entries.is_empty(), "router_updates has size but no entries");
        ...
        let near_10 = api
            .query_blocking(vec![10.0, 10.0], 1, 1)
            .expect("query");
